...
export function generateStaticParams() {
  return Object.entries(regions).flatMap(([city, districts]) => Object.entries(districts).flatMap(([district, neighborhoods]) => neighborhoods.map((neighborhood) => ({ city, district, neighborhood }))));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const area = await getArea(params);
  if (!area) return { title: "지역을 찾을 수 없습니다" };
  return { title: getTitle(area.city, area.district, area.neighborhood), description: getDescription(area.city, area.district, area.neighborhood) };
}

export default async function AreaPage({ params }: PageProps) {
  const area = await getArea(params);
  if (!area) notFound();
  const title = getTitle(area.city, area.district, area.neighborhood);
  const description = getDescription(area.city, area.district, area.neighborhood);
  const shops = [`${area.neighborhood} 프리미엄 출장마사지`, `${area.district} 홈케어 마사지`, `${area.city} 방문마사지 제휴센터`];
  return (
    <main className="page">
      <header className="topbar"><div className="topbar-inner"><a className="brand" href="/"><span className="brand-mark">K</span>케어링크</a><a className="nav-cta" href={`tel:${phoneNumber.replaceAll("-", "")}`}>{phoneNumber} 전화하기</a></div></header>
      <section className="section area-seo-page"><p className="eyebrow">{area.city} · {area.district} · {area.neighborhood}</p><h1>{title}</h1><p className="hero-copy">{description}</p><a className="primary-button" href={`tel:${phoneNumber.replaceAll("-", "")}`}>{phoneNumber} 바로 전화</a></section>
      <section className="section"><div className="section-heading"><h2>{area.neighborhood} 제휴 업체</h2><p>지역별 출장마사지 제휴 업체와 방문 가능 정보를 확인하세요.</p></div><div className="shop-list">{shops.map((shop) => <article className="shop-card" key={shop}><div className="shop-main"><div><span className="status-pill">서비스 가능</span><h3>{shop}</h3><p>{area.city} {area.district} {area.neighborhood}</p></div><div className="rating">4.8</div></div><div className="shop-meta"><span>오늘 전화 예약 가능</span><span>아로마 · 스포츠</span><strong>80,000원~</strong></div><a className="primary-button" href={`tel:${phoneNumber.replaceAll("-", "")}`}>{phoneNumber} 전화 연결</a></article>)}</div></section>
    </main>
  );
}
