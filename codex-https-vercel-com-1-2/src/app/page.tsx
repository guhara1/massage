...
const footerLinks = [
  ["수원 출장마사지", "/area/수원시/영통구/광교1동"], ["수원 인계동 출장마사지", "/area/수원시/팔달구/인계동"], ["분당 출장마사지", "/area/성남시/분당구/정자동"], ["성남 판교 출장마사지", "/area/성남시/분당구/판교동"], ["용인 기흥 출장마사지", "/area/용인시/기흥구/신갈동"], ["용인 수지 출장마사지", "/area/용인시/수지구/죽전1동"],
];
    ...
    <main className="page">
      <header className="topbar">
        <div className="topbar-inner">
          <a className="brand" href="#top"><span className="brand-mark"><Sparkles size={18} /></span>케어링크</a>
          <nav className="nav">
            <a href="#area">지역 선택</a>
            <a href="#partners">제휴업체</a>
            <a href="#shops">업체 목록</a>
            <a href="#admin">업체 등록</a>
      ...
      <section className="hero platform-hero" id="top">
        <div>
          ...
          <h1>시·구·동을 선택하면 가까운 제휴업체를 바로 확인합니다</h1>
          <p className="hero-copy">케어링크는 수원시, 용인시, 성남시 출장마사지 제휴업체를 행정구와 행정동 기준으로 정리하고 0508 대표번호로 바로 전화 연결을 제공합니다.</p>
          <div className="hero-actions">
            <a className="primary-button" href="#area"><MapPin size={18} />지역 선택하기</a>
            <a className="secondary-button" href="#partners"><BadgeCheck size={18} />제휴업체 보기</a>
            <a className="secondary-button" href={telLink}><PhoneCall size={18} />{phoneNumber}</a>
          </div>
        </div>
        <div className="quick-panel">
          <span className="panel-label">빠른 도시 선택</span>
          <div className="city-buttons">{Object.keys(regions).map((item) => <button className={item === city ? "city-button active" : "city-button"} key={item} onClick={() => selectCity(item)} type="button"><Building2 size={19} />{item}</button>)}</div>
          <div className="quick-result"><strong>{city}</strong><span>{district} · {neighborhood}</span><small>{affiliateShops.length}개 제휴 업체 노출 중</small></div>
        </div>
      </section>

      <section className="section" id="area">
        <div className="section-heading"><h2>3단계 지역 선택</h2><p>도시, 행정구, 행정동 순서로 누르면 해당 지역 출장마사지 업체 페이지와 전화 연결을 확인할 수 있습니다.</p></div>
        ...
        <div className="area-action-panel"><div><strong>{seoTitle}</strong><span>{seoDescription}</span></div><a className="primary-button" href={areaPagePath}>{neighborhood} 업체 페이지</a></div>
      </section>

      <section className="section" id="partners">
        <div className="section-heading"><h2>제휴업체 카테고리</h2><p>배너를 누르면 상담요청 없이 {phoneNumber} 번호로 바로 전화 연결됩니다.</p></div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 14 }} className="partner-grid">
          {partnerBanners.map((banner) => (
            <a href={telLink} key={banner.title} aria-label={`${banner.title} 바로 전화`} style={{ display: "grid", gap: 10, border: "1px solid var(--line)", borderRadius: 8, background: "#fff", overflow: "hidden", paddingBottom: 14 }}>
              <img src={banner.image} alt={banner.title} style={{ width: "100%", aspectRatio: "1 / 1", objectFit: "cover", display: "block" }} />
              <strong style={{ padding: "0 14px", fontSize: 17 }}>{banner.title}</strong>
              <span style={{ padding: "0 14px", color: "#667085", fontSize: 14 }}>{banner.area} · 바로 전화</span>
            </a>
          ))}
        </div>
        <div className="content-grid" style={{ marginTop: 16 }}>
          <article><h3>선정 경험</h3><p>지역별 실제 이용 흐름을 기준으로 응답 속도, 방문 가능 시간, 지역 적합성을 확인한 업체를 우선 노출합니다.</p></article>
          <article><h3>전문성 기준</h3><p>60분 9만원, 90분 12~15만원, 120분 VIP·스페셜 18만원 기준으로 코스와 가격을 명확히 표시합니다.</p></article>
          <article><h3>신뢰 정책</h3><p>합법적인 웰니스 방문 케어만 중개하며 불법 성매매, 유사 성행위, 미성년자 대상 서비스는 등록하지 않습니다.</p></article>
        </div>
      </section>

      <section className="section" id="shops">
        <div className="section-heading"><h2>{seoTitle}</h2><p>{seoDescription}</p></div>
        <div className="shop-list">{affiliateShops.map((shop) => <article className="shop-card" key={shop.id}><div className="shop-main"><div><span className="status-pill"><BadgeCheck size={15} />서비스 가능</span><h3>{shop.name}</h3><p>{shop.city} {shop.district} {shop.neighborhood}</p></div><div className="rating"><Star size={17} />{shop.rating}</div></div><div className="shop-meta"><span><Clock size={16} />{shop.available}</span><span>{shop.course}</span><strong>{shop.price}</strong></div><div className="tag-row">{shop.tags.map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div><a className="primary-button" href={telLink}><PhoneCall size={18} />{phoneNumber} 바로 전화</a></article>)}</div>
      </section>

      <section className="section" id="admin">
        <div className="section-heading"><h2>출장마사지샵 업데이트</h2><p>현재 선택된 지역에 새 제휴업체를 추가하는 운영자용 화면입니다. 정식 관리자 기능은 이후 저장소와 로그인 기능을 붙이면 완성됩니다.</p></div>
        <div className="admin-panel"><div className="admin-current"><MapPin size={22} /><div><strong>{city} {district} {neighborhood}</strong><span>이 지역에 새 제휴업체가 등록됩니다.</span></div></div><div className="admin-form"><label>샵 이름<input value={shopName} onChange={(e) => setShopName(e.target.value)} placeholder="예: 광교 프리미엄 홈케어" /></label><label>대표 코스<input value={shopCourse} onChange={(e) => setShopCourse(e.target.value)} /></label><label>가격<input value={shopPrice} onChange={(e) => setShopPrice(e.target.value)} /></label><button className="primary-button" onClick={addShop} type="button"><Plus size={18} />선택 지역에 추가</button></div></div>
      </section>

      <section className="section eeat-section">
        <div className="section-heading"><h2>E-E-A-T 운영 기준</h2><p>경험, 전문성, 권위성, 신뢰성을 강화하기 위해 지역 구조, 가격 기준, 전화 연결, 운영 정책을 명확하게 공개합니다.</p></div>
        <div className="eeat-grid"><article><strong>Experience</strong><h3>지역별 실제 흐름</h3><p>수원, 용인, 성남을 시·구·동 기준으로 나누어 고객이 실제 검색하는 지역 단위와 맞춥니다.</p></article><article><strong>Expertise</strong><h3>가격과 코스 공개</h3><p>60분 9만원, 90분 12~15만원, 120분 VIP·스페셜 18만원 기준으로 비교가 쉽도록 구성했습니다.</p></article><article><strong>Authoritativeness</strong><h3>고유 지역 페이지</h3><p>정자동, 인계동, 기흥, 판교 등 주요 지역은 고유 타이틀과 설명을 가진 상세 페이지로 연결됩니다.</p></article><article><strong>Trust</strong><h3>전화와 정책 공개</h3><p>모든 배너와 업체 카드에서 {phoneNumber} 바로 전화를 제공하고 합법 운영 정책을 반복 안내합니다.</p></article></div>
      </section>
